...
public class MyDingdanFragment extends BasicFragment{
	private volatile List<Appointment> newAppointmentList;
	private ListView newAppointmentLv;
	...
	private class RemoteDataTask extends AsyncTask<Void, Void, Void> {
	    ...
	    protected Void doInBackground(Void... params) {
	    	try {
				newAppointmentList = AppointmentDao.findAppointmentByID();
			} catch (AVException e) {
				e.printStackTrace();
			}
	    	return null;
	    ...
	    protected void onPostExecute(Void result) {
	      ...
	      newAppointmentLv = (ListView) getActivity().findViewById(R.id.zixun_dingdan_lv);
	      TextView noNetwork = (TextView) getActivity().findViewById(R.id.dingdan_empty_tv);
	      newAppointmentLv.setEmptyView(noNetwork);
		  
	      mAdapter = new DingdanAdapter(getActivity(), newAppointmentList);
	     
	      newAppointmentLv.setAdapter(mAdapter);
	      mAdapter.notifyDataSetChanged(); 
	      //注册上下文菜单
	      //registerForContextMenu(newProblemLv);

	      
	      if (newAppointmentList != null && !newAppointmentList.isEmpty()) {
	    	  noNetwork.setVisibility(View.GONE);
	      } else {
	    	  noNetwork.setVisibility(View.VISIBLE);
	      }
	      newAppointmentLv.setOnItemClickListener(new OnItemClickListener() {
				@Override
				public void onItemClick(AdapterView<?> listView, View itemView, int index, long id) {

					Appointment a = newAppointmentList.get(index);
					Bundle bundle = new Bundle();
					bundle.putSerializable("APPOINTMENT", a);
					UiHelper.showFragment(MyDingdanFragment.this, bundle, new AppointDetailFragment());
				}			
			});
	      newAppointmentLv.setOnScrollListener(new OnScrollListener() {
				@Override
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return UiHelper.getLightThemeView(getActivity(), R.layout.fragment_m_dingdan);
    }
