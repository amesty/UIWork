...
import com.example.adapter.TestCommentAdapter;
...
import com.example.dao.TestCommentDao;
...
public class CommentFragment extends BasicFragment{
	...
	private TextView testName,testGoodNum,testCommentNum;

	private TestCommentAdapter commentAdapter;
	private CommentReplyAdapter commentReplyAdapter;

	private static final int ONE_COMMENT_CODE = -1;

	private volatile List<CommentTest> commentTestList;
	private volatile List<TestReply> replyList;
	...
	private class RemoteDataTask extends AsyncTask<Void, Void, Void> {
	    ...
	    protected Void doInBackground(Void... params) {
	    	try {
	    		...
	    		commentTestList = (new TestCommentDao()).findTestCommentByType(testId);
			} catch (AVException e) {
	    ...
	    protected void onPostExecute(Void result) {
	      ...
	      lv_user_comment = (ListView) getActivity().findViewById(R.id.comment_lv);
	      TextView empty = (TextView) getActivity().findViewById(R.id.tv_empty);
	      lv_user_comment.setEmptyView(empty);
		  
	      //commentAdapter = new CommentAdapter(getContext(), newProblemList);
	      commentAdapter = new TestCommentAdapter(getActivity(), commentTestList,
					replyToCommentListener, commentReplyAdapter,
					goodToCommentListener, replyToReplyListener);
	      lv_user_comment.setAdapter(commentAdapter);
	      

	      if (commentTestList != null && !commentTestList.isEmpty()) {
	        empty.setVisibility(View.GONE);
	      } else {
	        empty.setVisibility(View.VISIBLE);
	      }
	      lv_user_comment.setOnScrollListener(new OnScrollListener() {
				@Override
			...
			setListViewHeightBasedOnChildren(lv_user_comment);
	    }
	}
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return UiHelper.getLightThemeView(getActivity(), R.layout.fragment_test_comment);
    }
    ...
    public void onViewCreated(View v, Bundle savedInstanceState) {
		...
		test_photo = (ImageView)v.findViewById(R.id.item_test_photo);
		testName = (TextView) v.findViewById(R.id.item_test_title);
		testGoodNum =  (TextView) v.findViewById(R.id.item_test_lookcount);
		testCommentNum = (TextView) v.findViewById(R.id.item_test_commentcount);
	...
	public void initView(View v) {
		...
		Picasso.with(getContext()).load(test.getTestPhotoUrl()).into(test_photo);
		testName.setText(test.getTestTitle());  
		testCommentNum.setText(test.getCommentNum()+"");
		testGoodNum.setText(test.getGoodNum()+"");
		...
		public void onClick(View v) {
			...
			btn_good.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					commentTestList.get(position).setGoodCount(commentTestList.get(position).getGoodNum()+1);
					commentTestList.get(position).saveInBackground();
				}
			});
		}
	};
			...
			public void onClick(View v) {
				...
				case ONE_COMMENT_CODE:
					if (TextUtils.isEmpty(edt_reply.getText().toString())) {
						...
						commenttest.setUser(currentUser);
						commenttest.setTest(test);
						commenttest.setGoodCount(0);
						commenttest.setContent(edt_reply.getText().toString());
						commenttest.saveInBackground();
						
						commentTestList.add(commenttest);
						commentAdapter.clearList();
						commentAdapter.updateList(commentTestList);
						commentAdapter.notifyDataSetChanged();
						customDialog.dismiss();
						edt_reply.setText("");
						Toast.makeText(getActivity(), "发表成功", Toast.LENGTH_SHORT).show();
						Test p = commenttest.getTest();
						p.setCommentCount(p.getCommentNum() + 1);
						p.saveInBackground();
					}
					break;
				default:
					if (TextUtils.isEmpty(edt_reply.getText().toString())) {
						...
						reply.setCommentTest(commentTestList.get(childPostion));
						reply.setContent(edt_reply.getText().toString());
						
						if (parentPositon != -1) {
							Answer_Reply ar = new Answer_Reply();
							ar.getCommentReplyList(commentTestList.get(parentPositon));
							try {
								List<TestReply> ReplyList = (List<TestReply>) ar.result.take();
							...
							Answer_Reply ar = new Answer_Reply();
							ar.getCommentReplyList(commentTestList.get(childPostion));
							try {
								//reply.setReplyTo("");
								reply.saveInBackground();
								...
								dao.setReplyList(commentTestList.get(childPostion), replyList);
							} catch (InterruptedException e) {
								e.printStackTrace();
							}
						}
