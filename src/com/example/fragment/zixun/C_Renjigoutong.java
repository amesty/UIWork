...
public class C_Renjigoutong extends BasicFragment{

	private volatile List<Counselor> RenjigoutongList;
	private ListView RenjigoutongLv;
	private CounselorAdapter mAdapter;
	
	private class RemoteDataTask extends AsyncTask<Void, Void, Void> {
	    @Override
	    protected Void doInBackground(Void... params) {
	    	RenjigoutongList = CounselorDao.findRenjigoutongByTime();
	    	return null;
	    }
	    ...
	    protected void onPostExecute(Void result) {
	      ...
	      RenjigoutongLv = (ListView) getActivity().findViewById(R.id.renjigoutong_lv);
	      TextView empty = (TextView) getActivity().findViewById(R.id.empty_tv);
	      RenjigoutongLv.setEmptyView(empty);
		  
	      mAdapter = new CounselorAdapter(getActivity(), RenjigoutongList);
	     
	      RenjigoutongLv.setAdapter(mAdapter);
	      mAdapter.notifyDataSetChanged(); 

	      if (RenjigoutongList != null && !RenjigoutongList.isEmpty()) {
	        empty.setVisibility(View.GONE);
	      } else {
	        empty.setVisibility(View.VISIBLE);
	      }
	      RenjigoutongLv.setOnItemClickListener(new OnItemClickListener() {
				@Override
				public void onItemClick(AdapterView<?> listView, View itemView, int index, long id) {

					Counselor p = RenjigoutongList.get(index);
					Bundle bundle = new Bundle();
					bundle.putSerializable("COUNSELOR", p);
					UiHelper.showFragment(C_Renjigoutong.this, bundle, new CounselorDetailFragment());
				}			
			});
	      RenjigoutongLv.setOnScrollListener(new OnScrollListener() {
				@Override
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return UiHelper.getLightThemeView(getActivity(), R.layout.fragment_c_renjigoutong);
    }
