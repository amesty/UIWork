...
public class C_Lianaihunyin extends BasicFragment {
	
	private volatile List<Counselor> LianaihunyinList;
	private ListView LianaihunyinLv;
	private CounselorAdapter mAdapter;
	
	private class RemoteDataTask extends AsyncTask<Void, Void, Void> {
	    @Override
	    protected Void doInBackground(Void... params) {
	    	LianaihunyinList = CounselorDao.findLianaihunyinByTime();
	    	return null;
	    }
	    ...
	    protected void onPostExecute(Void result) {
	      ...
	      LianaihunyinLv = (ListView) getActivity().findViewById(R.id.lianaihunyin_lv);
	      TextView empty = (TextView) getActivity().findViewById(R.id.empty_tv);
	      LianaihunyinLv.setEmptyView(empty);
		  
	      mAdapter = new CounselorAdapter(getActivity(), LianaihunyinList);
	     
	      LianaihunyinLv.setAdapter(mAdapter);
	      mAdapter.notifyDataSetChanged(); 

	      if (LianaihunyinList != null && !LianaihunyinList.isEmpty()) {
	        empty.setVisibility(View.GONE);
	      } else {
	        empty.setVisibility(View.VISIBLE);
	      }
	      LianaihunyinLv.setOnItemClickListener(new OnItemClickListener() {
				@Override
				public void onItemClick(AdapterView<?> listView, View itemView, int index, long id) {

					Counselor p = LianaihunyinList.get(index);
					Bundle bundle = new Bundle();
					bundle.putSerializable("COUNSELOR", p);
					UiHelper.showFragment(C_Lianaihunyin.this, bundle, new CounselorDetailFragment());
				}			
			});
	      LianaihunyinLv.setOnScrollListener(new OnScrollListener() {
				@Override
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return UiHelper.getLightThemeView(getActivity(), R.layout.fragment_c_lianaihunyin);
    }
