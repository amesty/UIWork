...
public class C_Zhichangwenti extends BasicFragment{

	private volatile List<Counselor> ZhichangwentiList;
	private ListView ZhichangwentiLv;
	private CounselorAdapter mAdapter;
	
	private class RemoteDataTask extends AsyncTask<Void, Void, Void> {
	    @Override
	    protected Void doInBackground(Void... params) {
	    	ZhichangwentiList = CounselorDao.findZhichangwentiByTime();
	    	return null;
	    }
	    ...
	    protected void onPostExecute(Void result) {
	      ...
	      ZhichangwentiLv = (ListView) getActivity().findViewById(R.id.zhichangwenti_lv);
	      TextView empty = (TextView) getActivity().findViewById(R.id.empty_tv);
	      ZhichangwentiLv.setEmptyView(empty);
		  
	      mAdapter = new CounselorAdapter(getActivity(), ZhichangwentiList);
	     
	      ZhichangwentiLv.setAdapter(mAdapter);
	      mAdapter.notifyDataSetChanged(); 

	      if (ZhichangwentiList != null && !ZhichangwentiList.isEmpty()) {
	        empty.setVisibility(View.GONE);
	      } else {
	        empty.setVisibility(View.VISIBLE);
	      }
	      ZhichangwentiLv.setOnItemClickListener(new OnItemClickListener() {
				@Override
				public void onItemClick(AdapterView<?> listView, View itemView, int index, long id) {

					Counselor p = ZhichangwentiList.get(index);
					Bundle bundle = new Bundle();
					bundle.putSerializable("COUNSELOR", p);
					UiHelper.showFragment(C_Zhichangwenti.this, bundle, new CounselorDetailFragment());
				}			
			});
	      ZhichangwentiLv.setOnScrollListener(new OnScrollListener() {
				@Override
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return UiHelper.getLightThemeView(getActivity(), R.layout.fragment_c_zhichangwenti);
    }
